# 알고리즘 - 자료구조 학습 / アルゴリズム-データ構造学習
알고리즘과 자료구조를 학습한 내용입니다.

アルゴリズムとデータ構造を勉強した内容です。


# 사용환경 / 使用環境
- C++23
- Visual Studio

# 학습내용 / 学習内容

## Data Structure
<details>
<summary><h3>List</h3></summary>

[**동적할당, 링크드리스트 함수화 (Dynamic Allocation, Linked List Functions)**](https://www.notion.so/Dynamic-Allocation-Linked-List-Functions-3519a595a8f5806b913ef5eec1f0fd98?pvs=21) 

</details>

<details>
<summary><h3>Stack</h3></summary>

[**스택, 큐 (Stack, Queue)**](https://www.notion.so/Stack-Queue-3519a595a8f5806da748d1e8f6bbf33f?pvs=21) 

</details>

<details>
<summary><h3>Queue</h3></summary>

[**스택, 큐 (Stack, Queue)**](https://www.notion.so/Stack-Queue-3519a595a8f5806da748d1e8f6bbf33f?pvs=21) 

</details>

<details>
<summary><h3>Graph, Tree</h3></summary>

[**그래프, 트리 (Graph, Tree)**](https://www.notion.so/Graph-Tree-3519a595a8f58090b5a8f2312177b724?pvs=21) 

[Tree](https://www.notion.so/Tree-2019a595a8f58009918bc3abd13ef49e?pvs=21) 

[Binary Tree](https://www.notion.so/Binary-Tree-2029a595a8f58073be00f9f875aacc35?pvs=21) 

</details>

<details>
<summary><h3>Hash</h3></summary>

[**Direct Addressing Table, Hash Table, 패턴 찾기 (DAT/Hash, Pattern Search)**](https://www.notion.so/Direct-Addressing-Table-Hash-Table-DAT-Hash-Pattern-Search-3519a595a8f5809d87ccde5873867995?pvs=21) 

</details>

<details>
<summary><h3>Heap</h3></summary>

[**최소/최대 힙, 이진 탐색 (Min/Max Heap, Binary Search)**](https://www.notion.so/Min-Max-Heap-Binary-Search-3519a595a8f58007bcc2c896b9aa79bb?pvs=21) 

[Heap / Priority Queue](https://www.notion.so/Heap-Priority-Queue-2069a595a8f580c38c35ce07ca67f733?pvs=21) 

</details>
  

# Algorithm

#### Binary Search Tree

[**이진 탐색 트리**](https://www.notion.so/3519a595a8f580dca63cf0eeb594f838?pvs=21) 

#### Direct

[**Direct 기법, 2중 포인터, 2차원 배열 패턴 찾기 (Direct, Double Ptr, 2D Pattern)**](https://www.notion.so/Direct-2-2-Direct-Double-Ptr-2D-Pattern-3519a595a8f58000b6a5f33d468de934?pvs=21) 

#### DFS, BFS

[**재귀 함수에 대한 사실과 오해 (Recursion: Facts & Myths)**](https://www.notion.so/Recursion-Facts-Myths-3519a595a8f580808809eb6bc54b67cc?pvs=21) 

[**재귀함수 호출의 깊이와 너비 (cstring, Recursion Depth/Width)**](https://www.notion.so/cstring-Recursion-Depth-Width-3519a595a8f580d89cf3c3fa151736b7?pvs=21) 

[**재귀함수 탐색 순서 (Recursion Traversal Order)**](https://www.notion.so/Recursion-Traversal-Order-3519a595a8f580efa6c6c16aa092ec20?pvs=21) 

[**재귀함수 가지치기 (Recursion Pruning)**](https://www.notion.so/Recursion-Pruning-3519a595a8f580f2b020fb6ccf1922a2?pvs=21) 

[**트리/그래프 DFS 순회 (Tree/Graph DFS Traversal)**](https://www.notion.so/DFS-Tree-Graph-DFS-Traversal-3519a595a8f580fab2dad868bf6c4872?pvs=21) 

[**트리/그래프 BFS 순회 (Tree/Graph BFS Traversal)**](https://www.notion.so/BFS-Tree-Graph-BFS-Traversal-3519a595a8f580f1b047cf372a3ebd3a?pvs=21) 

[**DFS/BFS 경로 탐색, 최소 여행비용 (Path Search, Min Travel Cost)**](https://www.notion.so/DFS-BFS-Path-Search-Min-Travel-Cost-3519a595a8f5802a8d2bc74c2dfac7fe?pvs=21) 

[**DFS 길찾기 알고리즘 (DFS Pathfinding)**](https://www.notion.so/DFS-DFS-Pathfinding-3519a595a8f5805d9db9ff5aa2995756?pvs=21) 

[**순열 알고리즘, DFS 경로찾기, 가중치 그래프 탐색 (Permutations, DFS Paths, Weighted Graph)**](https://www.notion.so/DFS-Permutations-DFS-Paths-Weighted-Graph-3519a595a8f58079ab85ecfda8eedba8?pvs=21) 

[NextPermutation](https://www.notion.so/NextPermutation-3199a595a8f5804486cacdd641597764?pvs=21) 

[**비트연산으로 조합풀기, BFS 경로 추적하기 (Bitmask Combinations, BFS Path Trace)**](https://www.notion.so/BFS-Bitmask-Combinations-BFS-Path-Trace-3519a595a8f58017910fcd0c4b59361e?pvs=21) 

#### Union-Find

[유니온 파인드 Union-Find](https://www.notion.so/Union-Find-3519a595a8f5801d9a9ada365c8823ce?pvs=21) 

#### MST, Kruskal

[**MST 최소신장 트리 Kruskal 알고리즘 (MST, Kruskal)**](https://www.notion.so/MST-Kruskal-MST-Kruskal-3519a595a8f5801bafa9f57c50425acc?pvs=21) 

#### Sort

[Bubble Sort](https://www.notion.so/Bubble-Sort-27d9a595a8f58016a296d982ebc28855?pvs=21) 

[Insertion Sort](https://www.notion.so/Insertion-Sort-27e9a595a8f580d58c62ead69751f407?pvs=21) 

[Selection Sort](https://www.notion.so/Selection-Sort-27d9a595a8f580b0bfd6c4cdb7174b52?pvs=21) 

[Heap Sort](https://www.notion.so/Heap-Sort-27e9a595a8f580bea4f3e87d9422b6a4?pvs=21) 

[Merge Sort](https://www.notion.so/Merge-Sort-2819a595a8f5809f8743f015865d9323?pvs=21) 

[**퀵 정렬 (Quick Sort)**](https://www.notion.so/Quick-Sort-3519a595a8f5805d8d4cdba0a9b18476?pvs=21) 

[**정렬 알고리즘(선택, 삽입, 버블, 합병, 힙, 퀵) (Sorting Algorithms)**](https://www.notion.so/Sorting-Algorithms-3519a595a8f5802f8781c0f3d8e9467c?pvs=21) 

[**unordered_map, 계수 정렬 (unordered_map, Counting Sort)**](https://www.notion.so/unordered_map-unordered_map-Counting-Sort-3519a595a8f5807fbf3bcc4e0efe6912?pvs=21) 

#### DP

[**다이나믹 프로그래밍 DP (Dynamic Programming)**](https://www.notion.so/DP-Dynamic-Programming-3519a595a8f580efa278d360fef04e2f?pvs=21) 

#### Dijkstra

[**다익스트라 알고리즘 (Dijkstra's Algorithm)**](https://www.notion.so/Dijkstra-s-Algorithm-3519a595a8f580e0990ef952d7641be4?pvs=21) 

#### AVL Tree

[**AVL 균형 이진 트리 구현하기 (AVL Tree)**](https://www.notion.so/AVL-AVL-Tree-3519a595a8f580d28612dcee8b8ed810?pvs=21) 

#### Red-Black Tree

[**레드-블랙 트리 구현하기 (Implement Red-Black Tree)**](https://www.notion.so/Implement-Red-Black-Tree-3519a595a8f580f5abd6ce9c81d25234?pvs=21)



<details>
<summary>클릭해서 펼치기</summary>

- Bit Masking
- Direct Array
- Hash Table
  - Direct Adressing Table
- Recursive Function
  - Path Tracking
  - Pruning
    - Prevention
    - Early Return
  - Tail Call
- Linked List
- Stack
- Queue
- Graph / Tree
  - Adjacency Matrix
- DFS
- BFS
- Permutation
  - Next Permutation
- Combitation
- Binary Search Tree
- Heap
  - Priority Queue
- Union Find
- MST - KRUSKAL Algorithm
- Sort
  - Bubble
  - Selection
  - Insertion
  - Merge
  - Heap


</details>
